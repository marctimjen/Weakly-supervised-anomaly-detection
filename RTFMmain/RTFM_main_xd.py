...
if __name__ == '__main__':
    ...
    for step in tqdm(range(0, param["max_epoch"]), total=param["max_epoch"], dynamic_ncols=True):
        ...
        run["val/loss"].log(val_loss/(param["xd_val_len"]//(param["batch_size"]*2)*param["batch_size"]))
        run["val/loss_cls"].log(loss_cls_sum/(param["xd_val_len"]//(param["batch_size"]*2)*param["batch_size"]))
        run["val/loss_sparse"].log(loss_sparse_sum/(param["xd_val_len"]//(param["batch_size"]*2)*param["batch_size"]))
        run["val/loss_smooth"].log(loss_smooth_sum/(param["xd_val_len"]//(param["batch_size"]*2)*param["batch_size"]))
        run["val/loss_rtfm"].log(loss_rtfm_sum/(param["xd_val_len"]//(param["batch_size"]*2)*param["batch_size"]))
