...
if __name__ == '__main__':
    ...
    for step in tqdm(range(0, param["max_epoch"]), total=param["max_epoch"], dynamic_ncols=True):
        ...
        run["val/loss"].log(
            loss / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))
        run["val/loss_sce"].log(
            sce_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))
        run["val/loss_mc"].log(
            mc_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))
        run["val/loss_smooth"].log(
            smooth_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))
        run["val/loss_sparse"].log(
            sparse_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))

        run["val/loss_con_sum"].log(
            con_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))
        run["val/loss_con_n_sum"].log(
            con_n_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))
        run["val/loss_con_a_sum"].log(
            con_a_sum / (param["xd_val_len"] // (param["batch_size"] * 2) * param["batch_size"]))

        val_info["epoch"].append(step)
        val_info["val_loss"].append(loss/(param["xd_val_len"]//(param["batch_size"]*2)*param["batch_size"]))
